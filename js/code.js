...
function addContact() {
  ...
  xhr.onreadystatechange = function () {
    ...
    try {
      ...
      resultEl.textContent = "Bad JSON from server.";
      return;
    }
...
function delContact() {
  const idStr = document.getElementById("contactIdToDelete").value.trim();
  const resEl = document.getElementById("contactDelResult");
  resEl.textContent = "";

  const contactId = Number(idStr);
  if (!Number.isInteger(contactId) || contactId <= 0) {
    resEl.textContent = "Enter a valid numeric Contact ID.";
    return;
  }
  if (!userId || userId <= 0) {
    resEl.textContent = "You appear to be logged out. Please sign in again.";
    return;
  }
  ...
  xhr.onreadystatechange = function () {
    ...
    if (xhr.status !== 200) {
      resEl.textContent = "Delete failed: " + xhr.status;
      return;
    }
    ...
    try {
      ...
      resEl.textContent = "Bad JSON from server.";
      return;
    }
    ...
    resEl.textContent = "Contact deleted.";
    document.getElementById("contactIdToDelete").value = "";
    
    searchContacts();
  };

  xhr.send(JSON.stringify({ contactId, userId }));
}
...
function searchContacts() {
  ...
  if (!userId || userId <= 0) {
    resultMsg.textContent =
      "You appear to be logged out. Please sign in again.";
    return;
  }
  ...
  xhr.onreadystatechange = function () {
    ...
    try {
      ...
      resultMsg.textContent = "Bad JSON from server.";
      return;
    }
    ...
    resultMsg.textContent = `Found ${results.length} contact(s).`;
  };
